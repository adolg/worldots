...
#include "uict.h"		/* HT_UIC_ACTT, uic */
...
/*@modifies fileSystem, stdout, errno@*/
{
	struct moveh *	last	= NULL;
	unsigned short	x, y;

	if (g->movehist->elemc > 0)
	{
		last	= & g->movehist->elems[g->movehist->elemc - 1];
	}
	...
	for (x = 0; x < g->rules->bwidth; x++)
	{
		if (printf("%2d ", (int)x) < 0)
		{
			return	HT_FR_FAIL_IO_PRINT;
		}
	}
	...
	for (y = 0; y < g->rules->bheight; y++)
	{
		if (printf("%6d\t", (int)y) < 0)
		{
			return	HT_FR_FAIL_IO_PRINT;
		}
		...
		for (x = 0; x < g->rules->bwidth; x++)
		{
			...
			const unsigned short pos = (unsigned short)
					(y * g->rules->bwidth + x);
			const HT_BIT_U8 bit	= g->b->pieces[pos];
			unsigned short ind	= type_index_get(bit);

			if (NULL != last
			&& (pos == last->pos || pos == last->dest))
			{
				chpad_l			= '[';
				chpad_r			= ']';
			}
...
/*@in@*/
/*@notnull@*/
	enum HT_UIC_ACTT * const HT_RSTR	act_sp,
/*@in@*/
/*@notnull@*/
...
/*@modifies * act, * act_sp, * done@*/
{
	if (HT_UI_ACT_CONTR == act->type && !act_contr_index)
	{
		 ...
		 */
		act->type	= HT_UI_ACT_SPECIAL;
		* act_sp	= HT_UIC_ACT_PRINT_PLAYERS;
		* done		= HT_TRUE;
	}
...
/*@modifies fileSystem, errno, stdout, stdin, * line, * act, * act_sp@*/
{
	...
	fr	= ht_line_readline(stdin, line);
	if (HT_FR_SUCCESS != fr)
	{
		return	fr;
	}
	...
	if (0 != ferror(stdin) || 0 != feof(stdin))
	{
		return		HT_FR_FAIL_IO_STDIN;
	}
...
/*@in@*/
/*@notnull@*/
	const enum HT_UIC_ACTT * const	act_sp
	)
...
/* splint incomplete logic */ /*@i1@*/\
	...
	{
		if (HT_UIC_ACT_PRINT_BOARD == * act_sp)
		{
			return	print_board(g);
		}
		else if (HT_UIC_ACT_PRINT_PLAYERS == * act_sp)
		{
...
{
	enum HT_FR		fr	= HT_FR_NONE;
	enum HT_UIC_ACTT	act_sp	= HT_UIC_ACT_UNK;
	...
	if (HT_AIT_NONE == playercur->contr)
	{
		fr	= cmd_human(g, line, act, & act_sp);
		if (HT_FR_SUCCESS != fr)
		{
			return	fr;
		}
	...
	)
{
	...
	do
	{
		...
		fr	= ht_line_readline(stdin, ui_c->line);
		if (HT_FR_SUCCESS != fr)
		{
			return	fr;
		}
		...
		if (!ht_line_empty(ui_c->line))
		{
			char ch	= ui_c->line->words[0].chars[0];
...
{
	if (NULL != g && NULL != ui_c && NULL != stop) {} /* -Wunused */
	return	HT_FR_SUCCESS;
}
	...
	)
{
	...
	while (ui_c->aiprog_symprtd < symtoprt)
	{
		unsigned short	tmp	= ui_c->aiprog_symprtd % 3;
		char		ch;
...
{
	struct uic * const	ui_c = malloc(sizeof(* ui_c));
	if (NULL == ui_c)
	{
		return	NULL;
	}

	ui_c->line	= alloc_ht_line((size_t)4, (size_t)11);
	if (NULL == ui_c->line)
	{
		free	(ui_c);
		return	NULL;
...
{
	if (NULL == g) {}	/* -Wunused */
	ui_c->aiprog_symprtd	= (unsigned short)0;
	return	HT_FR_SUCCESS;
}
